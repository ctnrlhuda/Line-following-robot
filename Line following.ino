...
{ 
   do
  {
    ...
    if ( sensorVal == LOW) { // if sensor detects obstacle, enter this condition
 ...
 else if( DS_2 ==1 && DS_3 ==1 || DS_1==1 && DS_2==1 )//01100-11000
  {
    left(100,100);
  }
  ...
  else if(DS_5 ==1)//00001
  {
   ...
   else if(DS_3 ==1 && DS_4 ==1 || DS_5==1 && DS_4==1)//00110-00011
   {
     right(100,150);
   }
  }
  } while(1); // continue to loop while the condition is true
}
...
//LineFollow();
  if (Serial.available() > 0) // condition when receiving data
  {
    button = Serial.read(); // button = read the incoming data
    startTime = millis();
   ...
   else if (button == 'Z') { // if data received equal to 'Z', servo moves up at angle 30°
     myservo.write(0);  
    }
  
   else if (button == 'X') { // if data received equal to 'X', servo moves down at angle 130°
      myservo.write(45); 
    }
   else if (button == 'C') { // if data received equal to 'C', servo moves down at angle 130°
      myservo.write(90); 
    }
 }
